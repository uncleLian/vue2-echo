...
export default new Router({
    ...
    mode: 'history',
    // 注意：因为页面的高度是获取数据后撑开的，所以需要开启Keep-alive，返回的时候scrollBehavior才生效。或者你不想keep-alive数据，使用下面注释的代码把滚动位置的值先存到路由meta里，然后在组件里获取，等数据回来后再自己$(window).scrollTop(position.y)实现滚动，这种场景我在写后台管理项目的时候用过，是可以的。
